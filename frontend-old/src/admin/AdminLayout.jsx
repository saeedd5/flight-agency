// //flight-agancy/frontend/src/admin/pages/AdminLayout.jsx
...
import { useState } from 'react';
import { Layout, Menu, Button, Avatar, Dropdown, theme, ConfigProvider, Select } from 'antd';
import {
  DashboardOutlined,
  UserOutlined,
  CalendarOutlined,
...
import { useTranslation } from '../contexts/TranslationContext'; // مسیر صحیح
...
export default function AdminLayout() {
  ...
  const getAntdLocale = () => (language === 'ar' ? arEG : enUS);

  const menuItems = [
    {
      key: 'dashboard', 
      icon: <DashboardOutlined />,
      // دقت کنید: /admin/ اضافه شد
      label: <Link to="/admin/dashboard">{t('dashboard')}</Link>, 
    },
    {
      key: 'air',
      icon: <ThunderboltOutlined />,
      label: <Link to="/admin/air">{t('air')}</Link>,
    },
    {
      key: 'users',
      icon: <UserOutlined />,
      label: <Link to="/admin/users">{t('userManagement')}</Link>,
    },
    {
      key: 'bookings',
      icon: <CalendarOutlined />,
      label: <Link to="/admin/bookings">{t('bookingManagement')}</Link>,
    },
    {
      key: 'logs',
      icon: <FileTextOutlined />,
      label: <Link to="/admin/logs">{t('searchLogs')}</Link>,
    },
    {
      key: 'settings',
      icon: <SettingOutlined />,
      label: <Link to="/admin/settings">{t('settings')}</Link>,
    },
  ];
  ...
  const userDropdownItems = [
    {
      key: 'home',
      icon: <HomeOutlined />,
      label: <Link to="/">{t('backToSite')}</Link>,
    },
    { type: 'divider' },
    {
      key: 'logout',
  ...
  return (
    <ConfigProvider
      ...
      locale={getAntdLocale()}
      theme={{
        token: { colorPrimary: '#157f43', borderRadius: 5 },
      }}
    >
      <Layout style={{ minHeight: '100vh', direction }}>
        <Sider
          ...
          className="admin-sider-white"
        >
          <div className="admin-logo">{collapsed ? 'T' : 'Travelyn'}</div>
          <Menu
            theme="light"
            mode="inline"
            selectedKeys={[selectedKey]}
        ...
        <Layout style={{ 
          ...
          <Header
            className="admin-main-header"
            style={{ padding: '0 24px', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}
          >
            <Button
              type="text"
              icon={collapsed ? <MenuUnfoldOutlined /> : <MenuFoldOutlined />}
          ...
          <Content
            className="admin-main-content"
            style={{ margin: '24px 16px', padding: 24, minHeight: 280, background: '#f5f5f5' }}
          >
            <Outlet /> {/* <-- مهمترین بخش: رندر کردن صفحات فرزند */}
          </Content>
        </Layout>
      </Layout>
    </ConfigProvider>
