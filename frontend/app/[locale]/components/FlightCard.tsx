          ...
          return (
            <Fragment key={index}>
              <div className="flex gap-3 text-xs">
                ...
                <div className="flex-1 text-left ltr:text-left rtl:text-right">
...
export function FlightCard({ flight }: { flight: FlightOption }) {
  const t = useTranslations('FlightCardRaw');
  const { user } = useAuth();
  ...
  const handleSaveFlight = async () => {
    ...
    if (res.success) {
      ...
      alert(res.errorMessage || t('modal.error'));
    }
    setIsSaving(false);
  };
  ...
  return (
    <>
      ...
      >
        <div className="flex flex-col md:flex-row min-h-[80px]">
          ...
          <div className="flex-1 p-3 flex flex-col justify-center">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className={`text-[9px] font-bold flex items-center gap-1 ${flight.seatsAvailable && flight.seatsAvailable <= 5 ? 'text-red-700' : 'text-emerald-700'}`}>
                <Users className="w-3 h-3" /> {flight.seatsAvailable && flight.seatsAvailable <= 5 ? t('left', {count: flight.seatsAvailable}) : t('available')}
              </div>
            </div>
              ...
              <div className="flex-1 flex flex-col items-center px-2">
                ...
                <div className="w-full relative flex items-center justify-center h-2">
                  ...
                  <div className={`z-10 px-2 py-0 text-[8px] font-bold bg-white border ${isDirect ? 'text-emerald-700 border-emerald-300' : 'text-amber-700 border-amber-300'}`}>
                    {isDirect ? t('direct') : t('stop', {count: flight.stops})}
                  </div>
                </div>
              </div>
          ...
          <div className="w-full md:w-[150px] bg-slate-50 p-3 border-t md:border-t-0 md:border-l border-slate-200 flex flex-row md:flex-col justify-between items-center md:items-end">
            <div className="text-left md:text-right">
              <div className="text-[9px] font-bold text-slate-400 uppercase">{t('price')}</div>
        ...
        {isExpanded && (
          <div className="bg-slate-50/50 border-t border-slate-200 p-4 grid grid-cols-1 md:grid-cols-2 gap-6 text-xs ltr:text-left rtl:text-right">
            <FlightSegmentsDetail segments={flight.segments} t={t} />
            
            <div>
              <h4 className="font-black text-slate-800 mb-3 flex items-center gap-2 text-[11px] uppercase tracking-tighter">
                 <ShieldAlert className="w-4 h-4 text-emerald-700" /> {t('fareBaggage')}
              </h4>
              <ul className="space-y-1 text-slate-600 font-medium">
                 <li className="flex justify-between border-b border-slate-200/50 pb-1"><span>{t('class')}</span> <span className="font-medium text-slate-900">{flight.cabinClass}</span></li>
                 <li className="flex justify-between border-b border-slate-200/50 pb-1"><span>{t('baggage')}</span> <span className="font-medium text-slate-900">{flight.baggageAllowance || t('notSpecified')}</span></li>
                 ...
                 <li className="flex justify-between border-b border-slate-200/50 pb-1">
                    <span>{t('baseFare')}</span> <span className="font-medium text-slate-900">${flight.basePrice?.toFixed(2) || 'N/A'}</span>
                 </li>
                 {flight.taxes?.map((tax, idx) => (
                    <li key={idx} className="flex justify-between border-b border-slate-200/50 pb-1">
                       <span>{t('tax', {code: tax.code})}:</span> <span className="font-medium text-slate-900">${tax.amount.toFixed(2)}</span>
                    </li>
                 ))}
              </ul>
            </div>
        ...
        <DialogContent className="rounded-none sm:max-w-[360px] border-emerald-800 p-0 shadow-2xl">
          <DialogHeader className="p-4 border-b border-slate-200 bg-emerald-950 text-white">
            <DialogTitle className="text-sm font-black uppercase tracking-tighter flex items-center gap-2">
              <ShieldAlert className="w-4 h-4 text-emerald-400" /> {t('modal.title')}
            </DialogTitle>
          </DialogHeader>
          <div className="p-5 space-y-5 bg-white">
            <div className="flex justify-between items-center bg-slate-50 p-3 border border-slate-200">
              <span className="text-[10px] font-bold uppercase text-slate-500">{t('modal.basePrice')}</span>
              <span className="font-black text-sm text-slate-800">${flight.price.toFixed(2)}</span>
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase text-slate-500">{t('modal.yourMarkup')}</label>
            ...
            <div className="flex justify-between items-center bg-emerald-50 p-3 border border-emerald-200">
              <span className="text-[10px] font-black uppercase text-emerald-800">{t('modal.finalPrice')}</span>
          ...
          <DialogFooter className="p-4 border-t border-slate-200 bg-slate-50">
            <Button onClick={handleSaveFlight} disabled={isSaving} className="w-full rounded-none bg-emerald-800 hover:bg-emerald-900 text-white h-10 text-xs font-black uppercase tracking-wider">
              {isSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Save className="w-4 h-4 mr-2" /> {t('modal.saveBtn')}</>}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
