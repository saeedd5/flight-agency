...
//   // 🔥 ۱. این استیت از ابتدا true است تا لودینگ در کسری از ثانیه نمایش داده شود 🔥
//   const [isSearching, setIsSearching] = useState(true); 
//   const [initialLoad, setInitialLoad] = useState(true);

//   // دریافت پارامترها به صورت مستقیم
//   const origin = searchParams.get('origin');
//   const destination = searchParams.get('destination');
//   const date = searchParams.get('date');
//   const hasSearched = !!(origin && destination && date);

//   // 🔥 ۲. هوکِ افکتِ بهینه شده (بدون وابستگی به کلِ شیء searchParams) 🔥
// useEffect(() => {
//     let isMounted = true;

//     const fetchData = async (o: string, d: string, dt: string) => {
//       setIsSearching(true);
//       // اضافه کردن تایم‌استمپ برای شکستن کش (که قبلا گفتم)
//       const urlDate = `${dt}&_t=${new Date().getTime()}`;
      
//       const res = await searchAgencyFlights({ origin: o, destination: d, date: urlDate });
...
//       fetchData(origin, destination, date);
//     } else {
//       // 🔥 حل قطعی مشکل کُندی تاریخ امروز 🔥
//       // با استفاده از یک Timeout صفر، به Next.js اجازه می‌دهیم پروسه Hydration را تمام کند
//       // و سپس تاریخ دقیق مرورگر کاربر را برای سرچ می‌فرستیم.
//       setTimeout(() => {
//         if (isMounted) {
//           const clientToday = format(new Date(), 'yyyy-MM-dd');
//           fetchData('ORD', 'ATL', clientToday);
//         }
//       }, 0);
//     }
...
//       isMounted = false;
//     };
//   }, [origin, destination, date, hasSearched]);
...
import { FlightSearchFormbooking } from './components/FlightSearchFormbooking';
import { FlightCardbooking } from './components/FlightCardbooking';
import { PlaneLanding, Globe2, Zap, ShieldCheck, Loader2, Sparkles, Building2, ChevronDown } from 'lucide-react';
import { format } from 'date-fns';
...
export default function HomePage() {
  ...
  const [flights, setFlights] = useState<any[]>([]); // استیت بلیت‌ها
  const [totalCount, setTotalCount] = useState(0); // کل بلیت‌های موجود در سرور
  ...
  // لود اولیه (صفحه اول - ۱۰ تای اول)
  useEffect(() => {
    let isMounted = true;
    const fetchInitialData = async (o: string, d: string, dt: string) => {
      setIsSearching(true);
      setPage(1); // ریست کردن صفحه
      
      const res = await searchAgencyFlights({ origin: o, destination: d, date: dt, page: 1, pageSize: 10 });
    ...
    if (hasSearched) {
      fetchInitialData(origin, destination, date);
    } else {
      const today = format(new Date(), 'yyyy-MM-dd');
      setTimeout(() => {
        if (isMounted) fetchInitialData('ORD', 'ATL', today);
      }, 0);
    }
    return () => { isMounted = false; };
  }, [origin, destination, date, hasSearched]);

  // 🔥 لود کردن صفحه بعدی با دکمه (بسیار سریع) 🔥
  const loadMoreFlights = async () => {
    setIsLoadingMore(true);
    const nextPage = page + 1;
    
    const o = hasSearched ? origin : 'ORD';
    const d = hasSearched ? destination : 'ATL';
    const dt = hasSearched ? date : format(new Date(), 'yyyy-MM-dd');
    
    const res = await searchAgencyFlights({ origin: o!, destination: d!, date: dt!, page: nextPage, pageSize: 10 });
    
    if (res.success) {
      setFlights(prev => [...prev, ...res.flights]); // اضافه کردن ۱۰ بلیت جدید به لیست قبلی
      setPage(nextPage);
    }
  ...
  return (
    <main className="min-h-screen bg-slate-50 pb-12 font-sans text-sm">
      ...
      {/* ... Hero Section و فرم جستجو (مثل قبل) ... */}
      <div className="bg-emerald-950 pt-16 pb-24 px-4 text-center relative overflow-hidden">
        <h1 className="text-3xl md:text-4xl font-black text-white mb-4 tracking-tight">{t('Hero.title')}</h1>
        <p className="text-emerald-100 text-sm max-w-xl mx-auto font-medium mb-8">{t('Hero.subtitle')}</p>
      </div>
      ...
      <div className="container mx-auto px-4 mt-8 flex justify-center items-start gap-4">
        ...
        <aside className="hidden lg:flex w-52 sticky top-6 self-start flex-shrink-0">
          <div className="bg-white border border-slate-200 rounded-none h-[60rem] w-full flex items-center justify-center shadow-sm border-l-4 border-l-emerald-800">
            <span className="text-slate-200 font-black text-2xl uppercase tracking-[0.6em] [writing-mode:vertical-lr] rotate-180">Ads</span>
          </div>
        </aside>
        ...
        <section className="w-full max-w-4xl flex-shrink-0">
          {isSearching ? (
             <div className="py-20 flex flex-col items-center justify-center bg-white border border-slate-200 shadow-sm">
               <Loader2 className="w-10 h-10 animate-spin text-emerald-800 mb-4" />
               <p className="font-black text-emerald-900 uppercase tracking-widest text-[10px]">{t('Loading.findingTickets')}</p>
             </div>
          ) : flights && (
            <div className="space-y-3">
              <div className="bg-white p-3 border border-slate-200 flex justify-between items-center shadow-sm rounded-none">
                ...
                {/* حالا totalCount را از بک‌اند میخوانیم */}
                <div className="bg-emerald-50 px-2 py-1 text-emerald-800 font-black text-[10px] border border-emerald-100 uppercase tracking-wider">
                  {totalCount} {t('Results.ticketsCount')}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-3">
                {flights.length > 0 ? (
                  <>
                    ...
                    {flights.length < totalCount && (
                      <Button 
                        ...
                        className="w-full h-12 bg-white border border-slate-300 text-emerald-800 hover:bg-emerald-50 hover:text-emerald-900 font-black uppercase tracking-widest rounded-none shadow-sm transition-colors mt-4"
                      >
                        {isLoadingMore ? <Loader2 className="w-4 h-4 animate-spin" /> : <><ChevronDown className="w-4 h-4 mr-2" /> Load More Flights</>}
                      </Button>
                    )}
                  </>
                ) : (
                  <div className="text-center py-20 bg-white border border-slate-200 shadow-sm rounded-none">
                    <PlaneLanding className="h-10 w-10 text-slate-200 mx-auto mb-3" />
                    <h3 className="font-black text-slate-800 text-xs uppercase tracking-widest mb-1">{t('Empty.noTicketsFound')}</h3>
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">
                      {hasSearched ? t('Empty.searchedDesc', { origin: origin || '', destination: destination || '' }) : t('Empty.featuredDesc')}
                    </p>
                  </div>
                )}
              </div>
        ...
        <aside className="hidden lg:flex w-52 sticky top-6 self-start flex-shrink-0">
          <div className="bg-white border border-slate-200 rounded-none h-[60rem] w-full flex items-center justify-center shadow-sm border-r-4 border-r-emerald-800">
            <span className="text-slate-200 font-black text-2xl uppercase tracking-[0.6em] [writing-mode:vertical-lr]">Ads</span>
          </div>
        </aside>
